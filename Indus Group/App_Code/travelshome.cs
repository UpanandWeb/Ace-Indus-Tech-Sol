...
public class travelshome
{
    ...
    public void insertfeedback(string name, string email, string content, string date,int sts)
    {
    ...
    public void insertfooter(string page, string title, string content, string file)
    {
        SqlCommand cmd = new SqlCommand("insert into IT_FooterPage(page,title,content,imagename,status) values('" + page + "','" + title + "','" + content + "','" + file + "',1) ", con);
        con.Open();
        cmd.ExecuteNonQuery();
        con.Close();
    }
